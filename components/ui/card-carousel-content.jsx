  ...
  return (
    <>
      ...
      <div className='lg:w-3/4 leading-8 p-4 lg:p-8 flex-col justify-between h-full' >
        <p className='2xl:h-auto h-40 xl:overflow-auto overflow-scroll lg:pb-8 pb-4' dangerouslySetInnerHTML={{ __html: content[index].text }}></p>

        <div className='lg:absolute lg:bottom-0 flex gap-8 xl:my-8 my-4'>

          <button className={`flex items-center justify-center w-10 h-10 rounded-full bg-white15  ${index === 0 ? 'opacity-50' : isTouchDevice() ? 'hover:opacity-75' : ""} ${showButtons === true ? 'flex' : 'hidden'}`} onClick={() => { if (index > 0) { setIndex(index - 1) } }}>
            <FaArrowLeft className='w-6 h-6 fill-white75' />
          </button>

          <button className={`flex items-center justify-center w-10 h-10 rounded-full bg-white15  ${index === content.length - 1 ? 'opacity-50' : isTouchDevice() ? 'hover:opacity-75' : ""} ${showButtons === true ? 'flex' : 'hidden'}`} onClick={() => { if (index < content.length - 1) { setIndex(index + 1) } }}>
